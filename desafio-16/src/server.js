...
const { infoLogger,  warnLogger } = require("./utils/logger");
...
if (args.mode === "cluster" && cluster.isPrimary) {
    ...
    cluster.on("exit", (worker) => {
        console.log(`Cluster ${worker.PID} has close`);
    });
} else {
    const app = express();
    ...
    const server = httpServer.listen(PORT, () => {
        console.log(
            `>>>>>Server started at http://localhost:${PORT} - Process ID ${PID} - Mode ${args.mode}`
            )
    });
